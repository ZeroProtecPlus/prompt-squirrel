...
export function categoryDatabaseExceptionHandler<T>(effect: Effect.Effect<T, DatabaseException>): Effect.Effect<T, ServiceException> {
    return effect.pipe(
        Effect.catchTag(DATABASE_EXCEPTION_TAG, (error: DatabaseException) =>
            Effect.gen(function* () {
                ...
                if (error.code === "SQLITE_CONSTRAINT") return yield* Effect.fail(new ServiceException("Category already exists", { cause: error }));
